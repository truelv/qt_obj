...
QByteArray QtTelnetAuthNull::authStep(const QByteArray &data)
{
    ...
    const uchar buf[8] = {Common::IAC, Common::SB, Common::Authentication,
                   Common::IS, Auth::AUTHNULL, 0, // CLIENT|ONE-WAY
                   Common::IAC, Common::SE};
    setState(AuthSuccess);
    return QByteArray((const char*)buf, sizeof(buf));
}
...
public:
    ...
    void sendCommand(const uchar operation, const uchar option);
...
void QtTelnetPrivate::parseSubTT(const QByteArray &data)
{
    ...
    const uchar c1[4] = { Common::IAC, Common::SB,
                         Common::TerminalType, Common::IS};
    sendCommand((const char*)c1, sizeof(c1));
    sendString("UNKNOWN");
    const uchar c2[2] = { Common::IAC, Common::SE };
    sendCommand((const char*)c2, sizeof(c2));
}
...
void QtTelnetPrivate::sendWindowSize()
{
    ...
    const uchar c[9] = { Common::IAC, Common::SB, Common::NAWS,
                        (w & 0x00ff), (w >> 8), (h & 0x00ff), (h >> 8),
                        Common::IAC, Common::SE };
    sendCommand((const char*)c, sizeof(c));
}
...
void QtTelnetPrivate::sendCommand(const uchar operation, const uchar option)
{
    const uchar c[3] = { Common::IAC, operation, option };
    sendCommand((const char*)c, 3);
}
...
void QtTelnet::sendControl(Control ctrl)
{
    ...
    const uchar command[2] = {Common::IAC, c};
    d->sendCommand((const char*)command, sizeof(command));
    if (sendsync)
        sendSync();
}
